...
export function DoctorDetails({ route, navigation }) {
  ...
  const handleMarcarConsulta = async () => {
    try {
      ...
      console.log(error);
      Alert.alert('Erro ao marcar a consulta, verifique se todos os dados estão digitados de forma correta.');
    }
  };
  ...
  const handleCancelar = () => {
    setSelectedDate(null);
    setSelectedTime(null);
    setShowButtons(false); // Cancela a seleção e oculta os botões
  };

  useEffect(() => {
    // Mostra os botões somente quando a data e a hora forem selecionadas
            ...
            {showButtons ? ( // Mostra os botões apenas quando showButtons for true
              <>
                <AppointmentButton onPress={handleMarcarConsulta}>
                  <AppointmentButtonText>Confirmar Consulta</AppointmentButtonText>
                </AppointmentButton>
              ...
              <>
                <View>
                  <Button title="Selecionar Data da Consulta" onPress={openDateModal} />
                </View>

                <View>
                  <Button title="Selecionar Hora da Consulta" onPress={openTimeModal} />
                </View>
              </>
            )}
          </View>
